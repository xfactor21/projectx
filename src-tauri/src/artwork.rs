...
fn classify(name: &str, relative: &str) -> (String, i32) {
    ...
    if rel.contains("src/assets") || rel.contains("src\\assets") {
        score += 8;
    }
    if rel.contains("icons/") || rel.contains("icons\\") {
        score += 8;
    }
    if lower.contains("512") || lower.contains("256") {
        score += 4;
    }
...
pub(crate) fn discover_project_artwork(
    ...
    for file in files {
        let Some(mime) = image_mime(&file) else {
            continue;
        };
        let relative = file
            .strip_prefix(&root)
