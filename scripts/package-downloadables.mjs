...
if (!existsSync(bundleDir)) throw new Error('The NSIS bundle directory does not exist. Build the v2.8 installer first.')
const installers = readdirSync(bundleDir)
  .filter((name) => /^project\.X_2\.8\.0_x64-setup\.exe$/i.test(name))
  .map((name) => join(bundleDir, name))
if (installers.length !== 1) throw new Error(`Expected exactly one project.X 2.8.0 NSIS installer, found ${installers.length}.`)
