...
const basename = 'projectX-08.28-v2.6-companion-launch'
...
if (!existsSync(bundleDir)) throw new Error('The NSIS bundle directory does not exist. Build the v2.6 installer first.')
const installers = readdirSync(bundleDir)
  .filter((name) => /^project\.X_2\.6\.0_x64-setup\.exe$/i.test(name))
  .map((name) => join(bundleDir, name))
if (installers.length !== 1) throw new Error(`Expected exactly one project.X 2.6.0 NSIS installer, found ${installers.length}.`)
