...
const basename = 'projectX_8-26_v1.9'
...
if (!existsSync(bundleDir)) throw new Error('The NSIS bundle directory does not exist. Build the v1.9 installer first.')
const installers = readdirSync(bundleDir)
  .filter((name) => /^project\.X_1\.9\.0_x64-setup\.exe$/i.test(name))
  .map((name) => join(bundleDir, name))
if (installers.length !== 1) throw new Error(`Expected exactly one project.X 1.9.0 NSIS installer, found ${installers.length}.`)
