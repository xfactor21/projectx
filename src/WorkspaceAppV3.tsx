...
export default function WorkspaceAppV3(){
  ...
  async function runProject(project:Project){
    ...
    try{const result=await desktop.runDevProject(source.path,script);setStatus(result.output)}catch(error){setStatus(error instanceof Error?error.message:'Unable to start project.')}
  }
